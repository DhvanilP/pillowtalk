...
/* Prototypes */
static pt_response_t *
http_operation(const char *method, const char *server_target, const char *data, unsigned data_len);

static pt_response_t *
http_operation_cookie(const char *method, const char *server_target, const char *data, unsigned data_len,
                      const char *cookie);
...
pt_response_t *pt_put_cookie(const char *server_target, pt_node_t *doc, const char *cookie) {
    char *data = NULL;
    int data_len = 0;
    pt_response_t *res = 0;
    if (doc) {
    ...
    res = http_operation_cookie("PUT", server_target, data, data_len, cookie);
    res->root = parse_json(res->raw_json, res->raw_json_len);
    if (data)
        free(data);
    return res;
...
pt_response_t *pt_get_cookie(const char *server_target, const char *cookie) {
    pt_response_t *res = http_operation_cookie("GET", server_target, NULL, 0, cookie);
    res->root = parse_json(res->raw_json, res->raw_json_len);
    return res;
}
...
static pt_response_t *http_operation_cookie(const char *http_method,
                                            const char *server_target, const char *data, unsigned data_len,
                                            const char *cookie) {
    CURL *curl_handle;
    CURLcode ret;
...
static void generate_node_json(pt_node_t *node, yajl_gen g) {
    if (node) {
        switch (node->type) {
            ...
            case PT_STRING:
                yajl_gen_string(g, (const unsigned char *) ((pt_str_value_t *) node)->value,
                                strlen(((pt_str_value_t *) node)->value));
                break;
